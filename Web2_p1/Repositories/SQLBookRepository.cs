...
namespace Web2_p1.Repositories
{
    public class SQLBookRepository : IBookRepository
    {
        ...
        // CẬP NHẬT: Triển khai GetAllBooks với Filter, Sort và Pagination (6 tham số)
        public List<BookWithAuthorAndPublisherDTO> GetAllBooks(
            ...
            int pageSize = 1000)
        {
            // 1. Tạo truy vấn cơ sở dưới dạng IQueryable để áp dụng các điều kiện sau
            ...
            }).AsQueryable(); // Quan trọng: Sử dụng AsQueryable()

            // 2. FILTERING (Lọc)
            if (string.IsNullOrWhiteSpace(filteron) == false &&
                string.IsNullOrWhiteSpace(filterQuery) == false)
            {
                // Chỉ áp dụng lọc theo trường "title"
                if (filteron.Equals("title", StringComparison.OrdinalIgnoreCase))
                {
                    // Lọc những sách có Title chứa filterQuery [cite: 46]
            ...
            {
                // Chỉ áp dụng sắp xếp theo trường "title" [cite: 99]
                if (sortBy.Equals("title", StringComparison.OrdinalIgnoreCase))
                {
                    // Sắp xếp tăng dần hoặc giảm dần dựa vào isAscending [cite: 102, 103]
            ...
            var skipResults = (pageNumber - 1) * pageSize; // Tính số bản ghi cần bỏ qua [cite: 170, 171, 172]

            // Bỏ qua (Skip) và Chỉ lấy (Take) số lượng bản ghi cho trang hiện tại 
